...
-- 患者向け紹介文（下書き。管理画面のスタッフ管理から編集できます）
update public.staff set bio = coalesce(bio, '院長・柔道整復師。スポーツ外傷から日常の痛み・不調まで幅広く対応します。米国NABOSO認定ニューロセンサリースペシャリストとして、足裏や神経の視点も取り入れた施術を行います。川西整体院も担当。') where name = '阿部';
update public.staff set bio = coalesce(bio, '副院長・鍼灸師。鍼灸の専門知識を活かし、痛みや自律神経の乱れ、慢性的な不調にもアプローチします。お身体の状態を丁寧に見立ててご提案します。') where name = '澁谷';
update public.staff set bio = coalesce(bio, '柔道整復師。ケガの回復から姿勢づくりまでサポートします。体幹教室も担当し、お子さまの運動指導にも力を入れています。') where name = '萩原';
update public.staff set bio = coalesce(bio, '柔道整復師。お一人おひとりの状態や目標に合わせた施術を心がけています。体幹教室も担当しています。') where name = '林';
