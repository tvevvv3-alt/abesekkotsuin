...
select '全身通電', 4, 1
where not exists (select 1 from public.equipment where name in ('全身通電','ハイチャージ'));
-- 既存の「ハイチャージ」は「全身通電」に改称
update public.equipment set name = '全身通電' where name = 'ハイチャージ';
...
-- 料金メモ（月謝制・加算など。カードに表示）
update public.services set price_note = coalesce(price_note,'月4回 ¥4,400 ／ フリーパス ¥8,150（月額制）') where name = '体幹教室';
update public.services set price_note = coalesce(price_note,'通常料金＋（学生 ¥550 ／ 一般 ¥2,750）')       where name = '時間外予約';

-- 川西整体院：毎正時のみ受付（10〜20時の毎正時。13-15時も開けられるようコマは用意し、
--   使わない日はその枠を「休診にする」で閉じる）。勤務時間に縛られない固定枠。
update public.services
   set after_hours = true, class_starts = '600,660,720,780,840,900,960,1020,1080,1140,1200'
 where name = '川西整体院';
