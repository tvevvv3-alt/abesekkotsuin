-- 管理画面からの予約・変更は「勤務時間／営業時間」も無視して入れられるようにする。
-- （カレンダーで空き時間をタップ→そのまま時間外でも予約できる／ドラッグで自由に移動できる運用に合わせる）
-- 患者向けの予約は従来どおり勤務時間内のみ（p_ignore_hours の既定は false）。
-- Supabase の SQL Editor で1回実行（再実行しても安全）。

-- 旧6引数版を破棄（7引数＋既定値と衝突するため）
...
-- ★ 予約確定：source='admin' のときは休診＋勤務時間を無視
