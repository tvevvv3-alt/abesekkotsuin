...
export default function BookingWizard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cancelled) return;
      if (liff.isLoggedIn()) {
        // ログイン済み → 本人トークンを取得（サイレント連携）
        const token = liff.getIDToken();
        if (token) setLiffIdToken(token);
      }
      // 未ログイン時に liff.login() を呼ぶと access.line.me で
      // 「Invalid redirect custom scheme」400 になり予約が止まるため呼ばない。
      // 連携は完了画面のOAuth（https コールバック）にフォールバックする。
    })().catch(() => {
    ...
    <div className="mx-auto min-h-screen max-w-md bg-white pb-24 shadow-sm">
      ...
      {step === 2 && service && (
        ...
        >
          ...
          {/* 時間外への導線（時間外メニュー・体幹教室・パーソナルでは出さない）。その場で下に展開。 */}
          {!afterHours && !isClass && !isPersonal && afterHoursService && (
            <div className="mt-3">
              <button
